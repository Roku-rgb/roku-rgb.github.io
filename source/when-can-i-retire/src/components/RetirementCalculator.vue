...
<template>
  <div class="page">
    ...
    <!-- Result Banner -->
    <ResultBanner :retire-age="result.retireAge" :retire-details="result.retireDetails" />
    ...
    <div class="grid-2">
      <SliderGroup title="個人 / 儲蓄" color="#60a5fa">
        <SliderInput v-model="inputs.currentAge.value" label="目前年齡" :min="20" :max="65" :step="1" unit=" 歲" />
        <SliderInput v-model="inputs.currentCash.value" label="當前現金（存款）" :min="0" :max="3000" :step="10" unit=" 萬" :format="fmtMoney" />
        <SliderInput v-model="inputs.monthlySave.value" label="每月存款" :min="0" :max="20" :step="0.5" unit=" 萬" />
        <SliderInput v-model="inputs.rSave.value" label="儲蓄期投資報酬率" :min="0" :max="16" :step="0.5" unit="%" />
      </SliderGroup>

      <SliderGroup title="退休 / 支出" color="#f59e0b">
        <SliderInput v-model="inputs.endAge.value" label="規劃活到" :min="70" :max="100" :step="1" unit=" 歲" />
        <SliderInput v-model="inputs.eBase.value" label="每年基本開支" :min="12" :max="120" :step="2" unit=" 萬" />
        <SliderInput v-model="inputs.eExtra.value" label="每年額外花費" :min="0" :max="60" :step="2" unit=" 萬" />
        <SliderInput v-model="inputs.pension.value" label="年金收入" :min="0" :max="60" :step="2" unit=" 萬/年" />
      </SliderGroup>
    </div>
    ...
    <div class="grid-3">
      ...
      <SliderGroup title="&nbsp;" color="#a78bfa">
        <SliderInput v-model="inputs.rRp.value" label="股市實質報酬率（RP）" :min="2" :max="16" :step="0.5" unit="%" />
      </SliderGroup>
    </div>
    ...
    <!-- Footer -->
    <div class="footer">
      所有金額為實質值（已扣除通膨） ・ LMP 以 TIPS 階梯建構 ・ RP 以 ABW 攤還式提領 ・ 本工具僅供參考，不構成投資建議
    </div>
  </div>
</template>
